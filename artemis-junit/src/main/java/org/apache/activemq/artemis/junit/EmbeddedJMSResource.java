...
public class EmbeddedJMSResource extends ExternalResource {
    ...
    public EmbeddedJMSResource(int serverId) {
        ...
        configuration = new ConfigurationImpl()
                .setName(SERVER_NAME + "-" + serverId)
                .setPersistenceEnabled(false)
                .setSecurityEnabled(false)
                .addAcceptorConfiguration(new TransportConfiguration(InVMAcceptorFactory.class.getName(), props));
    ...
    public Queue getDestinationQueue(String destinationName) {
        ...
        if (destination.isQueue()) {
            queue = jmsServer.getActiveMQServer().locateQueue(SimpleString.toSimpleString("jms.queue." + destination.getName()));
        } else {
            String topicAddress = "jms.topic." + destination.getName();
            BindingQueryResult bindingQueryResult = null;
            try {
                bindingQueryResult = jmsServer.getActiveMQServer().bindingQuery(SimpleString.toSimpleString(topicAddress));
            } catch (Exception ex) {
                log.error(String.format("getDestinationQueue( %s ) - bindingQuery for %s failed", destinationName, topicAddress), ex);
                return null;
            }
