   ...
   public void stop() {
      ...
      if (jmsServer != null) {
         try {
            ...
            log.warn(String.format("Exception encountered stopping %s: %s - ignoring",
                                   jmsServer.getClass().getSimpleName(), this.getServerName()),
                     ex);
         }
      }
   ...
   public Queue getDestinationQueue(String destinationName) {
      ...
      if (destination.isQueue()) {
         ...
         try {
            ...
            log.error(String.format("getDestinationQueue( %s ) - bindingQuery for %s failed", destinationName,
   ...
   public List<Queue> getTopicQueues(String topicName) {
      ...
      if (!destination.isQueue()) {
         ...
         try {
            ...
            log.error(String.format("getTopicQueues( %s ) - bindingQuery for %s failed", topicName,
                                    destination.getAddress()),
                      ex);
            return queues;
         }
