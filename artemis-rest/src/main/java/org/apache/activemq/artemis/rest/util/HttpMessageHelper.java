...
import static org.apache.activemq.artemis.rest.HttpHeaderProperty.JMS_PROPERTY_DISCRIMINATOR;
...
public class HttpMessageHelper {
   ...
   public static boolean isJmsProperty(String key) {
      return key.toLowerCase().startsWith( JMS_PROPERTY_DISCRIMINATOR );
   }
   ...
   public static void writeHttpMessage(HttpHeaders headers, byte[] body, ClientMessage message) throws Exception {
      ...
      for (Entry<String, List<String>> entry : hdrs.entrySet()) {
         ...
         if (isTransferableHttpHeader(key) || isJmsProperty( key )) {
