...
public class LogAnnotationProcessor extends AbstractProcessor {
               ...
               for (Element el : annotatedType.getEnclosedElements()) {
                  if (el.getKind() == ElementKind.METHOD) {
                     ...
                     if (messageAnnotation != null) {
                        checkRetiredIDs(bundleAnnotation, messageAnnotation.id());
                     ...
                     if (logAnnotation != null) {
                        checkRetiredIDs(bundleAnnotation, logAnnotation.id());
   ...
   void checkRetiredIDs(LogBundle bundleAnnotation, long id) {
      if (bundleAnnotation.retiredIDs() != null && !bundleAnnotation.retiredIDs().isEmpty()) {
         if (Arrays.stream(bundleAnnotation.retiredIDs().split(","))
                   .map(String::trim)
                   .map(Long::valueOf)
                   .collect(Collectors.toList()).contains(id)) {
            throw new IllegalArgumentException("Code ID " + id + " has been retired. Select a different ID.");
         }
      }
   }
