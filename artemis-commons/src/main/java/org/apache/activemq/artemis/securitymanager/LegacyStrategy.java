...
class LegacyStrategy implements SecurityManagerCompatibility {
   ...
   private final Method doPrivilegedException;
   ...
   LegacyStrategy(ReflectiveStrategy.Loader loader) throws ClassNotFoundException, NoSuchMethodException {
      ...
      doPrivilegedException = accessController.getDeclaredMethod("doPrivileged", PrivilegedExceptionAction.class);
   ...
   public <T> T doPrivileged(PrivilegedExceptionAction<T> action) throws PrivilegedActionException {
      return (T) ReflectiveStrategy.invoke(doPrivilegedException, null, action);
   }
