...
public class PerDestinationStoreLimitTest extends JmsTestSupport {
   ...
   public void doTestDLQAfterBlock(ActiveMQDestination destination) throws Exception {
      ...
      MessageConsumer consumer = destination.isQueue() ? consumerSession.createConsumer(destination) : consumerSession.createDurableSubscriber((Topic) destination, "Durable");
   ...
   protected void waitForBlocked(final AtomicBoolean done) throws InterruptedException {
      while (true) {
         Thread.sleep(1000);
