...
public abstract class ScriptedCompatibilityTest {
   ...
   private static GroovyClassLoader createShellClassLoader(String side) {
      ...
      List<URL> baseClassPath;
      try {
         baseClassPath = Collections.list(ScriptedCompatibilityTest.class.getClassLoader().getResources(""));
      } catch (IOException e) {
         Assume.assumeNoException(e);
         baseClassPath = Collections.emptyList();
      }
   ...
   private static URL[] getClassPath(String side) {
      ...
      return Splitter.onPattern(File.pathSeparator).splitToList(classPath).stream().map(entry -> {
         try {
