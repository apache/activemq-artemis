...
public class QueueBrowserTest extends ActiveMQTestBase {
   ...
   public void testConsumerBrowserWithSelector() throws Exception {
      ...
      for (int i = 0; i < numMessages; i++) {
         ClientMessage message = createTextMessage(session, "m" + i);
         message.putIntProperty(SimpleString.toSimpleString("x"), i);
         producer.send(message);
      }

      ClientConsumer consumer = session.createConsumer(QUEUE, SimpleString.toSimpleString("x >= 50"), true);
   ...
   public void testConsumerBrowserWithStringSelector() throws Exception {
      ...
      ClientConsumer consumer = session.createConsumer(QUEUE, SimpleString.toSimpleString("color = 'RED'"), true);
   ...
   public void testConsumerMultipleBrowserWithSelector() throws Exception {
      ...
      ClientConsumer consumer = session.createConsumer(QUEUE, SimpleString.toSimpleString("x < 50"), true);
      ClientConsumer consumer2 = session.createConsumer(QUEUE, SimpleString.toSimpleString("x >= 50"), true);
      ClientConsumer consumer3 = session.createConsumer(QUEUE, null, true);
