...
public class JMSLVQTest extends JMSClientTestSupport {
   ...
   private static final String LVQ_CUSTOM_KEY_QUEUE_NAME = "LV_CUSTOM_KEY_QUEUE";
   private static final String LV_NON_DESTRUCTIVE_QUEUE_NAME = "LV_NON_DESTRUCTIVE_QUEUE";
   ...
   protected void addConfiguration(ActiveMQServer server) {
      ...
      server.getAddressSettingsRepository().addMatch(LV_NON_DESTRUCTIVE_QUEUE_NAME, new AddressSettings().setDefaultLastValueQueue(true).setDefaultNonDestructive(true));
   }
   @Override
   protected void createAddressAndQueues(ActiveMQServer server) throws Exception {
      ...
      //Add LVQ using Non Destructive enabled
      server.addAddressInfo(new AddressInfo(SimpleString.toSimpleString(LV_NON_DESTRUCTIVE_QUEUE_NAME), RoutingType.ANYCAST));
      server.createQueue(SimpleString.toSimpleString(LV_NON_DESTRUCTIVE_QUEUE_NAME), RoutingType.ANYCAST, SimpleString.toSimpleString(LV_NON_DESTRUCTIVE_QUEUE_NAME), null, true, false, -1, false, true);
   ...
   public void testLVQ(ConnectionSupplier producerConnectionSupplier, ConnectionSupplier consumerConnectionSupplier) throws Exception {
      ...
      testLVQNonDestructive(producerConnectionSupplier, consumerConnectionSupplier);
   }
   ...
   public void testLVQNonDestructive(ConnectionSupplier producerConnectionSupplier, ConnectionSupplier consumerConnectionSupplier) throws Exception {
      sendLVQ(producerConnectionSupplier, LV_NON_DESTRUCTIVE_QUEUE_NAME, Message.HDR_LAST_VALUE_NAME.toString());

      QueueBinding queueBinding = (QueueBinding) server.getPostOffice().getBinding(SimpleString.toSimpleString(LV_NON_DESTRUCTIVE_QUEUE_NAME));
      ...
      //Consume Once
      receiveLVQ(consumerConnectionSupplier, LV_NON_DESTRUCTIVE_QUEUE_NAME, Message.HDR_LAST_VALUE_NAME.toString());
      assertEquals("Ensure Message count", 1, queueBinding.getQueue().getMessageCount());
      ...
      QueueControl control = (QueueControl) server.getManagementService().getResource(ResourceNames.QUEUE + LV_NON_DESTRUCTIVE_QUEUE_NAME);
