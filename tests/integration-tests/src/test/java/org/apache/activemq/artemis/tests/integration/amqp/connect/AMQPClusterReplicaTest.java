...
public class AMQPClusterReplicaTest extends AmqpClientTestSupport {
   ...
   public void testReplicaWithCluster() throws Exception {
      ActiveMQServer node_1 = createNode1();
      ActiveMQServer node_2 = createNode2();
   ...
   private ActiveMQServer createNode1() throws Exception {
      ActiveMQServer node_1 = createServer(NODE_1_PORT, false);

      ClusterConnectionConfiguration clusterConfiguration = new ClusterConnectionConfiguration().setName("cluster").setConnectorName("node1").setMessageLoadBalancingType(MessageLoadBalancingType.ON_DEMAND).setStaticConnectors(Collections.singletonList("node2"));
   ...
   private ActiveMQServer createNode2() throws Exception {
      ActiveMQServer node_2 = createServer(NODE_2_PORT, false);

      ClusterConnectionConfiguration clusterConfiguration = new ClusterConnectionConfiguration().setName("cluster").setConnectorName("node2").setMessageLoadBalancingType(MessageLoadBalancingType.ON_DEMAND).setStaticConnectors(Collections.singletonList("node1"));
