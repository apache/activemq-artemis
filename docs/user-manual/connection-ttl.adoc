...
This is controlled by setting the `clientFailureCheckPeriod` parameter on the client's connection URI, e.g.:
...
The default `clientFailureCheckPeriod` on an "unreliable" connection (e.g. a Netty connection) is `30000` milliseconds (i.e. 30 seconds).
The default `clientFailureCheckPeriod` on a "reliable" connection (e.g. an in-vm connection) is `-1`.
