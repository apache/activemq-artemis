...
Upstream brokers or addresses do not need to be reconfigured. Just add the needed permissions to the address for the downstream broker. Same applies for downstream configurations.
...
This is used to mark if the upstream queue should be deleted once downstream disconnects and the delay and message count parameters have been met. It is useful in automation of the cleanup. You may wish to disable this, if you want messages to be queued for the downstream when disconnect no matter what.
