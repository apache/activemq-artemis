...
An optional key evaluated by `org.apache.activemq.artemis.core.remoting.impl.ssl.(CachingSSLContextFactory|CachingOpenSSLContextFactory)`
to cache created SSL contexts and avoid recreating. See [Configuring an SSLContextFactory](#Configuring an SSLContextFactory)
for more details.
...
If you have use `JDK` SSL provider (the default), you can configure which
`SSLContextFactory` to use.
Currently the following two implementations are provided:
- `org.apache.activemq.artemis.core.remoting.impl.ssl.DefaultSSLContextFactory` (loaded by the default)
...
Currently the following two implementations are provided:
- `org.apache.activemq.artemis.core.remoting.impl.ssl.DefaultOpenSSLContextFactory` (loaded by the default)
