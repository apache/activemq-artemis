...
disk-full-policy::
This attribute can have one of the following values: `DROP`, `FAIL` or `BLOCK` and determines what happens when a disk becomes full.
The default value is `BLOCK`.
If the value is `DROP` then further messages will be silently dropped.
If the value is `FAIL` then further messages will be dropped and an exception will be thrown on the client-side.
If the value is `BLOCK` then client message producers will block when they try and send further messages.
See the xref:flow-control.adoc#flow-control[Flow Control], xref:paging.adoc#paging[Paging] and xref:paging.adoc#monitoring-disk[Monitoring Disk] chapters for more info.
