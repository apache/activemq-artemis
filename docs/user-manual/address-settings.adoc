...
defines the initial number of elements of the intermediate message reference buffer (an `Object[]`), which is instantiated for each queue.
By default, this value is `8192` bytes if not explicitly configured. This must be a positive power of 2 (i.e. `0` is not an option).
