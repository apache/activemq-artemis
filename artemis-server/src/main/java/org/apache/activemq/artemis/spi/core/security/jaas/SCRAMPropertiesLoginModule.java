 ...
 * {@link PropertiesLoginModule}. It can either store the username-password in
 * plain text or in an encrypted/hashed form. the {@link #main(String[])} method
 * provides a way to prepare unencrypted data to be encrypted/hashed.
 */
public class SCRAMPropertiesLoginModule extends PropertiesLoader implements AuditLoginModule {
	 ...
	 * @throws GeneralSecurityException if any security mechanism is not available
	 *                                  on this JVM
	...
	public static void main(String[] args) throws GeneralSecurityException, ScramException {
		...
		System.out.println(username + " = " + PasswordMaskingUtil.wrap(
				data.salt + SEPARATOR + data.iterations + SEPARATOR + data.serverKey + SEPARATOR + data.storedKey));
	}
