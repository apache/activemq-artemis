...
public class HAProxyMessageEnforcer extends ByteToMessageDecoder {
   ...
   HAProxyMessageEnforcer(boolean requireHaProxyMessage, String acceptorName) {
   ...
   protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
      ...
      if (isProxyProtocol(in) != requireHaProxyMessage) {
         ActiveMQServerLogger.LOGGER.proxyProtocolViolation(SocketAddressUtil.toString(ctx.channel().remoteAddress()), acceptorName, requireHaProxyMessage, requireHaProxyMessage ? "did not use" : "used");
         ctx.close();
      }
   }

   private static boolean isProxyProtocol(ByteBuf in) {
