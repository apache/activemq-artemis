...
public final class FileConfigurationParser extends XMLConfigurationUtil {
   ...
   private static final String INTERMEDIATE_MESSAGE_BUFFER_INITIAL_SIZE = "intermediate-message-buffer-initial-size";
   ...
   protected Pair<String, AddressSettings> parseAddressSettings(final Node node) {
      ...
      for (int i = 0; i < children.getLength(); i++) {
         ...
         } else if (INTERMEDIATE_MESSAGE_BUFFER_INITIAL_SIZE.equalsIgnoreCase(name)) {
            addressSettings.setIntermediateMessageBufferInitialSize(POSITIVE_POWER_OF_TWO.validate(INTERMEDIATE_MESSAGE_BUFFER_INITIAL_SIZE, XMLUtil.parseInt(child)).intValue());
         }
      }
      return setting;
   }
