...
public class BridgeImpl implements Bridge, SessionFailureListener, SendAcknowledgementHandler, ReadyListener, ClientProducerFlowCallback {
   ...
   public synchronized void start() throws Exception {
      if (state == State.STARTING || state == State.STARTED || state == State.STOPPING || state == State.PAUSING) {
         logger.debug("Bridge {} state is {}. Ignoring call to start.", configuration.getName(), state);
         if (state == State.STOPPING || state == State.PAUSING) {
            throw ActiveMQMessageBundle.BUNDLE.bridgeOperationCannotBeExecuted(configuration.getName(), "started", state);
         } else {
            return;
         }
      }
   ...
   public synchronized void stop() throws Exception {
      if (state == State.STOPPING || state == State.STOPPED || state == State.PAUSING) {
         logger.debug("Bridge {} state is {}. Ignoring call to stop.", configuration.getName(), state);
         if (state == State.PAUSING) {
            throw ActiveMQMessageBundle.BUNDLE.bridgeOperationCannotBeExecuted(configuration.getName(), "stopped", state);
         } else {
            return;
         }
      }
   ...
   public synchronized void pause() throws Exception {
      if (state == State.STOPPING || state == State.STOPPED || state == State.PAUSING || state == State.PAUSED) {
         logger.debug("Bridge {} state is {}. Ignoring call to pause.", configuration.getName(), state);
         if (state == State.STOPPING || state == State.STOPPED) {
            throw ActiveMQMessageBundle.BUNDLE.bridgeOperationCannotBeExecuted(configuration.getName(), "paused", state);
         } else {
            return;
         }
      }
