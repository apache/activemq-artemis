...
public class MetricsManagerTest {
   ...
   public void testQueueMetrics(boolean enableMetrics) throws Exception {
      ...
      MetricsManager metricsManager = new MetricsManager(RandomUtil.randomString(), metricsConfiguration, addressSettingsRepository, null, temp -> {
      ...
      // test temp queue
      CountDownLatch tempTest = new CountDownLatch(1);
      metricsManager.registerQueueGauge(RandomUtil.randomString(), RandomUtil.randomString(), true, (builder) -> {
         tempTest.countDown();
      });
      assertEquals(enableMetrics, tempTest.await(latchTimeout, TimeUnit.MILLISECONDS));

      // test normal queue
      CountDownLatch test = new CountDownLatch(1);
      metricsManager.registerQueueGauge(RandomUtil.randomString(), RandomUtil.randomString(), false, (builder) -> {
         test.countDown();
      });
      assertEquals(enableMetrics, test.await(latchTimeout, TimeUnit.MILLISECONDS));
   }
}
