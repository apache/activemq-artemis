...
public class BasicMirrorController<T extends Link> {
   ...
   protected final byte remoteMirrorId;

   protected final byte localMirrorId;
   ...
   public static byte getRemoteMirrorID(Link link) {
      if ( link != null && link.getRemoteProperties() != null && link.getRemoteProperties().containsKey(AMQPMirrorControllerSource.BROKER_ID)) {
         return ((Number)link.getRemoteProperties().get(AMQPMirrorControllerSource.BROKER_ID)).byteValue();
      } else {
         return -1;
      }
   }

   public BasicMirrorController(ActiveMQServer server, byte remoteMirrorId) {
