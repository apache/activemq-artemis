...
/**
 * Management interface that is backed by an active federation producer
 * that was created when demand was applied to a matching address or queue
 * on the remote broker that is bringing messages to itself via federation.
 *
 * The federation producer sends messages for the given address or queue to
 * a remote federation consumer.
 */
public interface AMQPFederationProducerControl {
    ...
    * being federated from this server instance to the remote. For an Address federation
    ...
    * reside. For Address federation this is the actual subscription address whose
    * messages are being federated from the local address.
    *
   ...
   /**
    * Gets the local FQQN that comprises the address and queue where the remote producer
    * will be attached.
    *
   ...
   /**
    * Gets the routing type that will be requested when creating a producer on the
    * local server.
    *
    ...
    *
    * @return the filter string in use for the federation producer.
    */
    ...
    * this federation producer with messages to send to the remote.
    *
