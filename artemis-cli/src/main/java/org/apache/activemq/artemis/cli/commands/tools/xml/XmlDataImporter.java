...
public final class XmlDataImporter extends ConnectionConfigurationAbtract {
   ...
   private void createUndefinedQueue(String name) throws Exception {
      ClientSession.QueueQuery queueQuery = managementSession.queueQuery(SimpleString.of(name));
      if (!queueQuery.isExists()) {
         managementSession.createQueue(QueueConfiguration.of(name).setRoutingType(RoutingType.ANYCAST).setDurable(true).setAutoCreateAddress(true));
      }
   }
   ...
   private void sendMessage(List<String> queues, Message message) throws Exception {
      ...
      if (destination == null) {
         createUndefinedQueue(queues.get(0));
