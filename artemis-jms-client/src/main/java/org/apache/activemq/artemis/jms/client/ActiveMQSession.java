...
public class ActiveMQSession implements QueueSession, TopicSession {
   ...
   void createTemporaryQueue(ActiveMQDestination destination, RoutingType routingType, SimpleString queueName, SimpleString filter, ClientSession.AddressQuery addressQuery) throws ActiveMQException {
      ...
      setQueueAttributesIfNotSet(queueAttributes, routingType, filter, false);
      setDefaultsQueueAttributesIfNotSet(queueAttributes, addressQuery);
   ...
   void createSharedQueue(ActiveMQDestination destination, RoutingType routingType, SimpleString queueName, SimpleString filter, boolean durable, ClientSession.AddressQuery addressQuery) throws ActiveMQException {
      QueueAttributes queueAttributes = destination.getQueueAttributes() == null ? new QueueAttributes() : destination.getQueueAttributes();
      setQueueAttributesIfNotSet(queueAttributes, routingType, filter, durable);
      setDefaultsQueueAttributesIfNotSet(queueAttributes, addressQuery);
   ...
   private void setQueueAttributesIfNotSet(QueueAttributes queueAttributes, RoutingType routingType, SimpleString filter, boolean durable) {
   ...
   private void setDefaultsQueueAttributesIfNotSet(QueueAttributes queueAttributes, ClientSession.AddressQuery addressQuery) {
      ...
      if (queueAttributes.getExclusive() == null) {
         queueAttributes.setExclusive(addressQuery.isDefaultExclusive());
      }
      if (queueAttributes.getLastValue() == null) {
         queueAttributes.setLastValue(addressQuery.isDefaultLastValueQueue());
      }
      if (queueAttributes.getLastValueKey() == null) {
         queueAttributes.setLastValueKey(addressQuery.getDefaultLastValueKey());
      }
      if (queueAttributes.getNonDestructive() == null) {
         queueAttributes.setNonDestructive(addressQuery.isDefaultNonDestructive());
      }
      if (queueAttributes.getConsumersBeforeDispatch() == null) {
         queueAttributes.setConsumersBeforeDispatch(addressQuery.getDefaultConsumersBeforeDispatch());
      }
      if (queueAttributes.getDelayBeforeDispatch() == null) {
         queueAttributes.setDelayBeforeDispatch(addressQuery.getDefaultDelayBeforeDispatch());
      }
   }
