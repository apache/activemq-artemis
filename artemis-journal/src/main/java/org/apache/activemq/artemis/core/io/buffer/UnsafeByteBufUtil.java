...
final class UnsafeByteBufUtil {
    ...
    private static final void checkIndex(ByteBuf byteBuf,int index) {
        checkIndex(byteBuf,index, 1);
    }

    private static final void checkIndex(ByteBuf byteBuf, int index, int fieldLength) {
        ensureAccessible(byteBuf);
        checkIndex0(byteBuf,index, fieldLength);
    }

    private static final void ensureAccessible(ByteBuf byteBuf) {
        if (SystemPropertyUtil.getBoolean("io.netty.buffer.bytebuf.checkAccessible", true)
                && byteBuf.refCnt() == 0) {
            throw new IllegalReferenceCountException(0);
        }
    }

    private static final void checkIndex0(ByteBuf byteBuf, int index, int fieldLength) {
        if (isOutOfBounds(index, fieldLength, byteBuf.capacity())) {
            throw new IndexOutOfBoundsException(String.format(
                    "index: %d, length: %d (expected: range(0, %d))", index, fieldLength, byteBuf.capacity()));
